...
TURNS = {
  LEFT => {BACK => UP, FORWARD => DOWN},
  RIGHT => {BACK => DOWN, FORWARD => UP},
  DOWN => {BACK => RIGHT, FORWARD => LEFT},
  UP => {BACK => LEFT, FORWARD => RIGHT},
}
...
carts = grid.flat_map.with_index do |line, y|
...
loop do
  carts.sort_by! { |a| [a.y, a.x] }.each do |cart|
    next if cart.nil?
